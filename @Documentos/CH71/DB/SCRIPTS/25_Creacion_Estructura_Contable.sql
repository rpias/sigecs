...
--
/*
UPDATE roles SET nombre = 'CONTABLE' WHERE id_rol = 2;

INSERT INTO menu (menu, encabezado, padre, hijo_de, orden, ruta, icono, habilitado, id_usuario) 
VALUES ('Contabilidad', 1, 1,0,10,'#', 'fa fa-book', 1, 1);

INSERT INTO menu (menu, encabezado, padre, hijo_de, orden, ruta, icono, habilitado, id_usuario) 
VALUES ('Movimientos', 0, 0, 13,10,'contable_movimientos', 'fa fa-pencil', 1, 1);

INSERT INTO roles_menu (id_rol, id_menu) VALUES (1,13);
INSERT INTO roles_menu (id_rol, id_menu) VALUES (1,14);

INSERT INTO roles_menu (id_rol, id_menu) VALUES (2,13);
INSERT INTO roles_menu (id_rol, id_menu) VALUES (2,14);

SELECT * FROM menu;
*/

--
-- Estructura de tablas para CONTABILIDAD`
--
--
-- Estructura de tabla para la tabla `contable_rubros`
--
...
-- ----------------------------------------------------------------------------------------------------------------------
DROP procedure IF EXISTS `SP_ListarRubrosPadres`;
DELIMITER $$
USE `sigecs`$$
CREATE PROCEDURE `SP_ListarRubrosPadres` (
)
BEGIN 
	SELECT
    *
    FROM contable_rubros AS CR
    WHERE es_padre = 1 AND habilitado = 1;
END$$
DELIMITER ; 
...
-- SP_ListarSubRubros
-- ----------------------------------------------------------------------------------------------------------------------
DROP procedure IF EXISTS `SP_ListarSubRubros`;
DELIMITER $$
USE `sigecs`$$
CREATE PROCEDURE `SP_ListarSubRubros` (
	  id_rubro_padre int
)
BEGIN 
	SELECT
    *
    FROM contable_rubros AS CR
    WHERE id_padre = id_rubro_padre
    AND es_padre = 0 AND habilitado = 1;
END$$
DELIMITER ; 
